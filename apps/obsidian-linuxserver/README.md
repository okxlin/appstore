...
## 产品介绍
Obsidian 是一个 Markdown 笔记桌面应用，本应用通过 LinuxServer.io 镜像提供浏览器访问的 Obsidian 桌面环境。

## 主要功能
- 通过浏览器访问 Obsidian 桌面界面。
- 使用 `/config` 持久化用户目录、配置和笔记库。
...
## Introduction
Obsidian is a Markdown note-taking desktop app. This app packages the LinuxServer.io image as a browser-accessible Obsidian desktop environment.

## Features
- Access the Obsidian desktop from a browser.
- Persist the user home, settings, and vaults under `/config`.
- Protect the Web desktop entrypoint with Basic Auth.

## 应用简介
Obsidian 笔记桌面。

英文说明：Obsidian desktop app maintained by LinuxServer.io.
