...
LEGACY_SNIPE_IT_APP_KEY="base64:K4tN8COBEIginakyKQ4SGfoAskVW1leA/A0qSEDucFQ="
LEGACY_SNIPE_IT_DB_PASSWORD="snipeit-change-me"
...
APP_KEY_CACHE="${APP_KEY_CACHE:-${CONFIG_PATH_VALUE}/.snipe_it_app_key}"
DB_PASSWORD_CACHE="${DB_PASSWORD_CACHE:-${CONFIG_PATH_VALUE}/.snipe_it_db_password}"

if [[ -f "$ENV_FILE" ]]; then
  app_key="$(read_env_value APP_KEY || true)"
  cached_app_key="$(read_cache_value "$APP_KEY_CACHE" || true)"
  if laravel_app_key_is_valid "$app_key" && [[ "$app_key" != "$LEGACY_SNIPE_IT_APP_KEY" ]]; then
  ...
  if [[ -n "$db_password" && "$db_password" != "$LEGACY_SNIPE_IT_DB_PASSWORD" ]]; then
