...
- Reuse 1Panel store-managed MySQL or MariaDB runtimes through the install form selector
...
- 保留官方 `web + ci-server + worker` 三服务拓扑，但移除官方 compose 中自带的 MySQL 容器，改为复用 1Panel 商店管理的 MySQL / MariaDB 服务实例。
...
- 版本目录 `2.8.2` 与 `latest` 当前都固定到同一官方发布线：`jianmu-ui:v2.8.2`、`jianmu-server:v2.8.2`、`jianmu-worker-docker:v1.0.14`。
...
| 变量 | 说明 | 默认值 | 必填 |
| --- | --- | --- | --- |
| `PANEL_DB_TYPE` | 安装表单中的数据库类型选择器，支持 `mysql` / `mariadb` | `mysql` | 是 |
...
- 安装表单中请先选择数据库类型，再从下一级 service 列表选择具体的 1Panel 商店数据库实例；当前已适配 `MySQL` 和 `MariaDB`。
- 1Panel 会为 Jianmu 创建独立数据库和用户，并记录应用与数据库资源的关联；本包不会读取数据库 runtime 的 root 密码或直接操作其容器。
- 旧安装如果已经把 `PANEL_DB_HOST` 手工写成外部 MySQL 主机名或 IP，本次升级不会覆盖该值；这类外部数据库场景仍需提前手工创建 `PANEL_DB_NAME`、`PANEL_DB_USER` 和对应权限。
- 已经使用 `localmysql` 的旧安装会保留现有 `.env` 和数据库连接，不会被升级脚本改写；该兼容路径不具备 1Panel 数据库资源关联，新安装不再提供该选项。
...
- 当前包固定到官方 `v2.8.2` / `v1.0.14` 镜像线，`latest` 目录也同样固定到该发布线。
...
- `upgrade.sh` 只在字段缺失时把旧测试包里的 `DB_PORT` 迁移为 `PANEL_DB_PORT`，并根据旧 `PANEL_DB_HOST` 补齐 `PANEL_DB_TYPE`；它不会覆盖已有值或修改数据库内容。
