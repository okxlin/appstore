...
## 产品介绍
ONLYOFFICE Desktop Editors 是桌面办公编辑器，本应用通过 LinuxServer.io 镜像提供浏览器访问的 ONLYOFFICE 桌面环境。

## 主要功能
- 通过浏览器访问 ONLYOFFICE 桌面编辑器。
- 使用 `/config` 持久化用户目录、设置和文档。
...
## Introduction
ONLYOFFICE Desktop Editors provides office document editing tools. This app packages the LinuxServer.io image as a browser-accessible ONLYOFFICE desktop environment.

## Features
- Access ONLYOFFICE Desktop Editors from a browser.
- Persist the user home, settings, and documents under `/config`.
- Protect the Web desktop entrypoint with Basic Auth.

## 应用简介
ONLYOFFICE 桌面编辑器。

英文说明：ONLYOFFICE desktop editors maintained by LinuxServer.io.
