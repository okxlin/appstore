...
raw_data_dir="$(configured_value "APP_DATA_DIR" "./data")"
DATA_DIR="$(resolve_app_path "APP_DATA_DIR" "$raw_data_dir")"
[[ ! -L "$DATA_DIR" ]] || { echo "unsafe APP_DATA_DIR path" >&2; exit 1; }
for subdir in config files postgres redis; do
  subdir_path="$DATA_DIR/$subdir"
  [[ ! -L "$subdir_path" ]] || { echo "unsafe APP_DATA_DIR path" >&2; exit 1; }
  mkdir -p -- "$subdir_path"
  [[ ! -L "$DATA_DIR" && ! -L "$subdir_path" ]] || {
    echo "unsafe APP_DATA_DIR path" >&2
    exit 1
  }
done

chown -R --no-dereference 991:991 -- "$DATA_DIR/config" "$DATA_DIR/files" 2>/dev/null || true
