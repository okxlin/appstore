...
Firefox (Kasm) 将 Firefox 作为浏览器工作区运行。镜像由 Kasm Technologies 构建，1Panel 应用包直接使用固定 digest，不在本地重打包镜像。
...
- 本包固定 Kasm 官方 `1.19.0-rolling-weekly` 在 2026-07-22 发布的 amd64,arm64 digest `sha256:a3da1b453539c2683311c56d92ca6d7429fdd5bbf42b1070c050d88320823f39`，不会随远端标签静默变化。
- 2026-07-28 对此精确 digest 的 fresh Trivy 扫描结果表面为 `0 Critical / 4 High`，但 4 项均为版本映射误报。镜像实际安装 Firefox `152.0.6+build1-0ubuntu0.22.04.1~mt1`；Mozilla 已在 Firefox 97.0.2 修复 CVE-2022-26485/26486，另外 CVE-2022-25235/25236 实际属于 Expat 2.4.5 之前版本，却被扫描器错误归到 `firefox` 包。后续镜像或扫描数据库变化时仍须重新核验。
...
Firefox (Kasm) runs Firefox as a browser-accessible workspace. Kasm Technologies builds the image, and this package references a pinned digest without rebuilding it.
...
`APP_DATA_DIR` persists `/home/kasm-user`. The image is pinned to the 2026-07-22 `1.19.0-rolling-weekly` digest. Trivy reported `0 Critical / 4 High` on 2026-07-28, but all four are version-mapping false positives: the installed Firefox is 152.0.6, Mozilla fixed CVE-2022-26485/26486 in 97.0.2, and CVE-2022-25235/25236 are Expat issues incorrectly attributed to the Firefox package.

## References

- Product website: <https://www.mozilla.org/firefox/>
- Kasm official registry: <https://registry.kasmweb.com/1.1/>
- Docker Hub image and stand-alone instructions: <https://hub.docker.com/r/kasmweb/firefox>
- Pinned image source: <https://github.com/kasmtech/workspaces-images/blob/bafab6d531eefd5a5aa6f2c9088ebc8f02e12fae/dockerfile-kasm-firefox>
