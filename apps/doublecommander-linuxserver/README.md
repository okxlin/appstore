# Double Commander LinuxServer

## 产品介绍

Double Commander LinuxServer 使用 LinuxServer.io 维护的 `linuxserver/doublecommander` 镜像，提供 Double Commander 文件管理桌面 能力。

## 主要功能

- 提供 文件管理桌面 能力
- 持久化保存配置和业务数据
- 使用安装表单配置服务端口和数据路径
- 支持自定义时区

## 访问说明

安装完成后，通过 HTTP 或 HTTPS 端口访问 Double Commander Web 桌面。

## Introduction

Double Commander LinuxServer uses the LinuxServer.io maintained `linuxserver/doublecommander` image for file management desktop.

## Features

- Provide file management desktop
- Persist configuration and application data
- Configure service ports and data paths from the install form
- Configure the container time zone

## Access

After installation, open the Double Commander web desktop from the HTTP or HTTPS port.

## Links

- LinuxServer image documentation: <https://docs.linuxserver.io/images/docker-doublecommander/>
- Project website: <https://doublecmd.sourceforge.io/>
