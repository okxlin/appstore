...
[[ "$(id -u)" -eq 0 ]] || fail "Liwan init must run as root"

if [[ ${LIWAN_HOST_DATA_DIR+x} ]]; then
  DATA_DIR_RAW="$LIWAN_HOST_DATA_DIR"
else
  DATA_DIR_RAW="$(read_env_value LIWAN_HOST_DATA_DIR)"
fi
DATA_DIR_RAW="$(strip_matching_quotes "${DATA_DIR_RAW:-./data}")"

[[ -n "$DATA_DIR_RAW" ]] || fail "LIWAN_HOST_DATA_DIR must not be empty"
path_is_dotenv_safe "$DATA_DIR_RAW" || fail "LIWAN_HOST_DATA_DIR contains unsupported dotenv characters"

if [[ "$DATA_DIR_RAW" = /* ]]; then
  [[ ! -L "$DATA_DIR_RAW" ]] || fail "LIWAN_HOST_DATA_DIR must not be a symbolic link"
  DATA_DIR_ABS="$(realpath -m -- "$DATA_DIR_RAW")"
  [[ "$DATA_DIR_ABS" != "/" ]] || fail "LIWAN_HOST_DATA_DIR must not be the filesystem root"
else
  ...
  [[ ! -L "$DATA_DIR_PATH" ]] || fail "LIWAN_HOST_DATA_DIR must not be a symbolic link"
  ...
  case "$DATA_DIR_ABS" in
    ...
    *) fail "Relative LIWAN_HOST_DATA_DIR must remain inside the isolated retained-data directory" ;;
  esac

  set_env_value LIWAN_HOST_DATA_DIR "\"${DATA_DIR_ABS}\""
fi

if [[ -e "$DATA_DIR_ABS" && ! -d "$DATA_DIR_ABS" ]]; then
  fail "LIWAN_HOST_DATA_DIR must be a directory"
fi
...
[[ "$(stat -c '%u:%g' "$DATA_DIR_ABS")" == "1000:1000" ]] || \
  fail "LIWAN_HOST_DATA_DIR must be owned by UID/GID 1000:1000"
