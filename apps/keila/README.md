...
- 上游要求管理员密码至少 10 个字符。会话密钥必须是至少 64 个字符的单行 `.env` 安全值，建议只使用字母和数字，避免空格、引号、`$`、`#`、反斜杠及换行。
- 安装或升级时，如果 `SECRET_KEY_BASE` 少于 64 个字符，脚本会明确提示并将其替换为由系统安全随机源生成的 128 位十六进制值（仅含 `0-9a-f`）。达到 64 个字符的用户值会原样保留。
...
`SECRET_KEY_BASE` must be a single-line, dotenv-safe value of at least 64 characters. Alphanumeric values are recommended. During installation or upgrade, a shorter value is reported and replaced with a securely generated 128-character hexadecimal secret; valid user-provided values are preserved.
