...
## 产品介绍
RustDesk 是一个远程桌面客户端，本应用通过 LinuxServer.io 镜像提供浏览器访问的 RustDesk 桌面环境。

## 主要功能
- 通过浏览器访问 RustDesk 桌面客户端。
- 使用 `/config` 持久化用户目录和客户端配置。
...
## Introduction
RustDesk is a remote desktop client. This app packages the LinuxServer.io image as a browser-accessible RustDesk desktop environment.

## Features
- Access the RustDesk desktop client from a browser.
- Persist the user home and client settings under `/config`.
- Protect the Web desktop entrypoint with Basic Auth.

## 应用简介
RustDesk 远程桌面客户端。

英文说明：RustDesk desktop client maintained by LinuxServer.io.
