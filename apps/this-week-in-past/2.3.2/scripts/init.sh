...
ensure_dir() {
  ...
  mkdir -p -- "$path"
  [[ "$(resolve_app_path "$key" "$raw")" == "$path" ]] || fail "unsafe ${key} path"
  chmod 0750 "$path"
  chown -R --no-dereference 1337:1337 "$path"
}

ensure_dir "PHOTO_LIBRARY_PATH" "./data/photos"
ensure_dir "APP_DATA_DIR" "./data/app"
