...
Pinta (Kasm) 将 Pinta 作为浏览器工作区运行。镜像由 Kasm Technologies 构建，1Panel 应用包直接使用固定 digest，不在本地重打包镜像。
...
- 本包固定 Kasm 官方 `1.19.0-rolling-weekly` 在 2026-07-22 发布的 amd64,arm64 digest `sha256:0ad58675911c3f46fa52a7640492fd309e3f432f8bd5dc61dbc33ad56f0f93d9`，不会随远端标签静默变化。
- 2026-07-28 对此精确 digest 的 fresh Trivy 扫描结果为 `0 Critical / 5 High`。记录来自 `Tmds.DBus` 以及 `accountsservice`/`libaccountsservice0` 的两个重复 CVE。默认容器不挂载宿主 D-Bus、不共享宿主用户服务、移除全部 capabilities，且只向外提供 KasmVNC 6901，未确认远程请求可直接到达这些本地 D-Bus 路径；仍应只向可信用户开放，并在上游修复镜像发布后升级。
...
Pinta (Kasm) runs Pinta as a browser-accessible workspace. Kasm Technologies builds the image, and this package references a pinned digest without rebuilding it.
...
`APP_DATA_DIR` persists `/home/kasm-user`. The image is pinned to the 2026-07-22 `1.19.0-rolling-weekly` digest. A fresh Trivy scan on 2026-07-28 found `0 Critical / 5 High` in `Tmds.DBus` and `accountsservice` libraries. The container has no host D-Bus mount, shared host user service, or Linux capabilities, so no direct remote path was confirmed; restrict access to trusted users and upgrade when fixed upstream images are available.

## References

- Product website: <https://www.pinta-project.com/>
- Kasm official registry: <https://registry.kasmweb.com/1.1/>
- Docker Hub image and stand-alone instructions: <https://hub.docker.com/r/kasmweb/pinta>
- Pinned image source: <https://github.com/kasmtech/workspaces-images/blob/bafab6d531eefd5a5aa6f2c9088ebc8f02e12fae/dockerfile-kasm-pinta>
