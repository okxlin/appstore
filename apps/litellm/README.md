...
- 两个密钥均由 1Panel 安装表单随机生成并保存在应用配置中；新安装不再重复生成或改写它们。若从曾使用本地密钥缓存的旧包升级，兼容升级脚本会恢复旧值，避免已有凭据失效。
- 本包没有本地业务数据卷。备份或迁移时必须同时保存 PostgreSQL 数据库和 1Panel 应用配置，尤其要保持 `LITELLM_SALT_KEY` 不变。
