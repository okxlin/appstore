...
- 应用分类：工具。
- 支持架构：amd64、arm64。
- 商店同时提供 `latest` 和固定版本目录；升级前请备份配置与数据库数据。
...
## 安全说明
- HedgeDoc 主镜像包含 Node.js 应用及其依赖，镜像扫描器可能报告继承自这些依赖的 High 或 Critical 漏洞；固定版本标签不能消除已知风险。
- 本应用同时运行 MariaDB。请及时关注 LinuxServer/HedgeDoc 上游安全更新，升级前备份配置和数据库数据，仅向可信网络开放服务，并使用 HTTPS 与适当的访问控制。
