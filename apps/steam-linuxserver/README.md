...
## 产品介绍
Steam 是一个游戏分发和桌面客户端平台，本应用通过 LinuxServer.io 镜像提供浏览器访问的 Steam 桌面环境。

## 主要功能
- 通过浏览器访问 Steam 桌面界面。
- 使用 `/config` 持久化用户目录、配置和游戏文件。
...
## Introduction
Steam is a game distribution platform and desktop client. This app packages the LinuxServer.io image as a browser-accessible Steam desktop environment.

## Features
- Access the Steam desktop from a browser.
...
英文说明：Steam desktop client maintained by LinuxServer.io.
...
- 官方镜像需要 `seccomp:unconfined` 以支持游戏运行时的命名空间调用；这会降低 Docker 默认 seccomp 隔离强度。
...
- 文档: <https://docs.linuxserver.io/images/docker-steam/>
- 源码: <https://github.com/linuxserver/docker-steam>
